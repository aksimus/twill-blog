...
<body>
    ...
    <div class="mx-auto max-w-4xl px-5 md:px-0">
        ...
        <nav class="mt-16 mb-8" aria-label="Breadcrumb">
            <ol class="flex items-center space-x-2 text-sm text-gray-500">
                ...
                <li class="flex items-center">
                    ...
                    <span class="text-gray-900 font-medium">{{ __('Tag') }}: {{ $tag->title }}</span>
                </li>
            </ol>
        </nav>
        ...
        <!-- Tag Header -->
        <header class="mb-8">
            <h1 class="text-4xl font-bold text-gray-900 mb-4">{{ __('Tag') }}: {{ $tag->title }}</h1>
            ...
            <div class="text-sm text-gray-500">
                {{ __('Found') }} {{ $items->count() }} {{ __('posts tagged with') }} "{{ $tag->title }}" {{ __('yet.') }}
            </div>
        </header>
            ...
            <article class="mb-8 p-6 bg-white rounded-lg shadow-sm border border-gray-200 hover:shadow-md transition-shadow">
                <header class="mb-4">
                    ...
                    <div class="flex items-center gap-4 text-sm text-gray-500 mb-3">
                        ...
                        @if($post->blogTags && $post->blogTags->count() > 1)
                            <span class="flex items-center gap-1">
                                <span class="text-gray-400">•</span>
                                <span class="text-gray-600">{{ __('Other tags') }}:</span>
                                @foreach($post->blogTags->where('id', '!=', $tag->id) as $otherTag)
                                    <a href="{{ route('blog.tag', $otherTag->getSlug()) }}" 
                                       class="text-blue-600 hover:text-blue-800 transition-colors">
                                        {{ $otherTag->title }}
                                    </a>
                                    @if(!$loop->last), @endif
        ...
        @empty
            <div class="text-center py-12">
                ...
                <p class="text-gray-600">{{ __('No posts are tagged with') }} "{{ $tag->title }}" {{ __('yet.') }}</p>
            </div>
        @endforelse
            ...
            <div class="mt-8 pt-6 border-t border-gray-200">
                <div class="flex items-center justify-between">
                    ...
                    <div class="flex items-center space-x-2">
                        ...
                        @else
                            <a href="@localizedUrl('blog/tag/' . $tag->getSlug(), ['page' => $items->currentPage() - 1])" 
