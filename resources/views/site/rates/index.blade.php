    ...
    <div class="rates__control">
        <div class="rates__link-list">
            @foreach($availableYears as $year)
                <div class="rates__link {{ $year == $currentYear ? 'rates__link--selected' : '' }}">
                    <a href="{{ route('rates.show', ['year' => $year, 'quarter' => $currentQuarter]) }}" class="c-link">{{ $year }}</a>
                </div>
            @endforeach
        </div>
        <div class="rates__link-list">
            @foreach($quarters as $quarter)
                <div class="rates__link {{ $quarter == $currentQuarter ? 'rates__link--selected' : '' }}">
                    <a href="{{ route('rates.show', ['year' => $currentYear, 'quarter' => $quarter]) }}" class="c-link">
                        @switch($quarter)
                            @case(1)
